import { Boxes, Edit3, PackageOpen, Plus, RefreshCw } from 'lucide-react'
import { useCallback, useEffect, useRef, useState, type FormEvent } from 'react'
import { createInput, deactivateInput, listInputs, updateInput } from '../api/inventory'
...
export function InventoryPage() {
  ...
  return <>
    <section className="page-heading"><div><h1>Insumos</h1><p>Gestiona existencias, unidades de medida y niveles mínimos.</p></div>{canManage && <button className="primary" onClick={() => setEditing(null)}><Plus size={19}/>Nuevo insumo</button>}</section>
    ...
    <section className="catalog-card">
      {loading ? <div className="state" role="status"><span className="spinner"/>Cargando insumos…</div> : !items.length ? <div className="state"><PackageOpen size={40}/><b>Aún no hay insumos</b><span>Crea el primer registro para comenzar.</span></div> : <div className="table-scroll"><table><thead><tr><th>Insumo</th><th>Unidad</th><th>Stock actual</th><th>Stock mínimo</th><th>Nivel</th><th>Estado</th>{canManage && <th>Acciones</th>}</tr></thead><tbody>{items.map((item) => {
        const lowStock = item.activo && item.stock_actual <= item.stock_minimo
        return <tr key={item.id}><td className="entity-name"><span className="entity-icon"><Boxes size={18}/></span><span>{item.nombre}{item.descripcion && <small className="unit-price">{item.descripcion}</small>}</span></td><td>{item.unidad_medida}</td><td>{item.stock_actual}</td><td>{item.stock_minimo}</td><td><span className={`stock-level ${!item.activo ? 'inactive' : lowStock ? 'low' : 'ok'}`}>{!item.activo ? 'Sin seguimiento' : lowStock ? 'Stock bajo' : 'Disponible'}</span></td><td><span className={`badge ${item.activo ? 'active' : ''}`}>{item.activo ? 'Activo' : 'Inactivo'}</span></td>{canManage && <td className="actions"><button onClick={() => setEditing(item)}><Edit3 size={16}/>Editar</button><button onClick={() => void toggle(item)}>{item.activo ? 'Desactivar' : 'Activar'}</button></td>}</tr>
      })}</tbody></table></div>}
    </section>
