import { AlertTriangle, ArrowLeft, CalendarDays, CheckCircle2, CreditCard, PackageCheck, ReceiptText, Shirt, UserRound, XCircle } from 'lucide-react'
  ...
  return <section className="order-detail-page">
    ...
    <div className="order-detail-grid">
      ...
      <aside className="order-side-column">
        ...
        <section className="detail-card payment-summary"><header><ReceiptText size={20}/><h2>Pagos</h2>{canManage && paid < order.total && <Link className="payment-link" to={`/pedidos/${order.id}/cobrar`}><CreditCard size={16}/>Registrar pago</Link>}</header>{order.pagos.map(payment => <div key={payment.id}><span>{payment.metodo_pago.nombre}<small className="unit-price">{date.format(new Date(payment.fecha_pago))}{payment.referencia ? ` · ${payment.referencia}` : ''}</small></span><b>{money.format(payment.monto)}</b></div>)}{!order.pagos.length && <p className="empty-block">Sin pagos registrados.</p>}<div><span>Pagado</span><b>{money.format(paid)}</b></div><div><span>Saldo pendiente</span><strong>{money.format(Math.max(order.total - paid, 0))}</strong></div></section>
      </aside>
    </div>
